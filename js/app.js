...
btn.onclick = () => {
    let day_val = document.getElementById("day")
    let month_val = document.getElementById("month")
    let year_val = document.getElementById("year")
    let current_data = new Date()
    let differenceInMs = current_data - new Date(year_val.value, month_val.value - 1, day_val.value)
  ...
  const daysLeft = Math.floor((differenceInMs % (1000 * 60 * 60 * 24 * 30.44)) / (1000 * 60 * 60 * 24));
    console.log(ageInYears, monthsLeft, daysLeft)
}
*/
